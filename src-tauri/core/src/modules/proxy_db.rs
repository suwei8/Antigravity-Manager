use crate::modules::proxy::monitor::ProxyRequestLog;
...
pub fn get_stats() -> Result<crate::modules::proxy::monitor::ProxyStats, String> {
    let db_path = get_proxy_db_path()?;
    let conn = Connection::open(db_path).map_err(|e| e.to_string())?;
    ...
    Ok(crate::modules::proxy::monitor::ProxyStats {
        total_requests,
        success_count,
        error_count,
    })

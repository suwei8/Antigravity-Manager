...
pub fn get_data_dir() -> Result<PathBuf, String> {
    // Check if we are in a Flatpak, but wait, we are removing Flatpak dependency.
    // So default to ~/.local/share/antigravity-tools OR standard XDG paths.
    // For now align with CLI which uses:
    if let Ok(dir) = std::env::var("ANTIGRAVITY_DATA_DIR") {
        return Ok(PathBuf::from(dir));
    }

    let home = std::env::var("HOME").map_err(|_| "HOME not set".to_string())?;
    let data_dir = PathBuf::from(home).join(".local/share/antigravity-tools");
...
pub async fn fetch_quota_with_retry(account: &mut Account) -> crate::error::AppResult<QuotaData> {
    ...
    // 检查是否需要刷新 Token
    let need_refresh = match &result {
        Err(AppError::Network(e)) => e.status() == Some(StatusCode::UNAUTHORIZED),
        Err(AppError::Unknown(msg)) => msg.contains("401") || msg.contains("Unauthorized"),
        _ => false,
    };

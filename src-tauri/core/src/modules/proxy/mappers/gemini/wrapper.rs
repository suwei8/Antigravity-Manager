...
pub fn wrap_request(body: &Value, project_id: &str, mapped_model: &str) -> Value {
    ...
    let config = crate::modules::proxy::mappers::common_utils::resolve_request_config(original_model, final_model_name, &tools_val);
                    ...
                    if let Some(decls_arr) = decls.as_array_mut() {
                        ...
                        for decl in decls_arr {
                            if let Some(params) = decl.get_mut("parameters") {
                                crate::modules::proxy::common::json_schema::clean_json_schema(params);
                            }
                        }
                    }
                }

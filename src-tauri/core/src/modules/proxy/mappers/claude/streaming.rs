...
// use crate::modules::proxy::mappers::signature_store::store_thought_signature; // Deprecated
use crate::modules::proxy::SignatureCache;
use bytes::Bytes;
use serde_json::json;
    ...
    fn process_function_call(
        ...
        let tool_id = fc.id.clone().unwrap_or_else(|| {
            format!(
                "{}-{}",
                fc.name,
                crate::modules::proxy::common::utils::generate_random_id()
            )
        });

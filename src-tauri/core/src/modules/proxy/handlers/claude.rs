...
use crate::modules::proxy::mappers::claude::{
    transform_claude_request_in, transform_response, create_claude_sse_stream, ClaudeRequest,
    close_tool_loop_for_thinking,
};
use crate::modules::proxy::server::AppState;
...
use crate::modules::proxy::mappers::claude::models::{ContentBlock, Message, MessageContent};
...
fn determine_retry_strategy(
    ...
    match status_code {
        ...
        429 => {
            ...
            if let Some(delay_ms) = crate::modules::proxy::upstream::retry::parse_retry_delay(error_text) {
...
pub async fn handle_messages(
    ...
    let zai_enabled = zai.enabled && !matches!(zai.dispatch_mode, crate::modules::proxy::ZaiDispatchMode::Off);
    ...
    let use_zai = if !zai_enabled {
        ...
        match zai.dispatch_mode {
            crate::modules::proxy::ZaiDispatchMode::Off => false,
            crate::modules::proxy::ZaiDispatchMode::Exclusive => true,
            crate::modules::proxy::ZaiDispatchMode::Fallback => google_accounts == 0,
            crate::modules::proxy::ZaiDispatchMode::Pooled => {
    ...
    let mut request: crate::modules::proxy::mappers::claude::models::ClaudeRequest = match serde_json::from_value(body) {
        Ok(r) => r,
        Err(e) => {
            return (
    ...
    if use_zai {
        ...
        return crate::modules::proxy::providers::zai_anthropic::forward_anthropic_json(
            &state,
    ...
    let meaningful_msg = request.messages.iter().rev()
        ...
        .find_map(|m| {
            let content = match &m.content {
                crate::modules::proxy::mappers::claude::models::MessageContent::String(s) => s.to_string(),
                crate::modules::proxy::mappers::claude::models::MessageContent::Array(arr) => {
                    // 对于数组，提取所有 Text 块并拼接，忽略 ToolResult
                    arr.iter()
                        .filter_map(|block| match block {
                            crate::modules::proxy::mappers::claude::models::ContentBlock::Text { text } => Some(text.as_str()),
                            _ => None,
                        })
                        .collect::<Vec<_>>()
                        .join(" ")
    ...
    let latest_msg = meaningful_msg.unwrap_or_else(|| {
        request.messages.last().map(|m| {
            match &m.content {
                crate::modules::proxy::mappers::claude::models::MessageContent::String(s) => s.clone(),
                crate::modules::proxy::mappers::claude::models::MessageContent::Array(_) => "[Complex/Tool Message]".to_string()
            }
    ...
    for (idx, msg) in request.messages.iter().enumerate() {
        let content_preview = match &msg.content {
            crate::modules::proxy::mappers::claude::models::MessageContent::String(s) => {
                if s.len() > 200 {
    ...
    for attempt in 0..max_attempts {
        // 2. 模型路由解析
        let mut mapped_model = crate::modules::proxy::common::model_mapping::resolve_model_route(
        ...
        let config = crate::modules::proxy::mappers::common_utils::resolve_request_config(&request_for_body.model, &mapped_model, &tools_val);

        // 0. 尝试提取 session_id 用于粘性调度 (Phase 2/3)
        // 使用 SessionManager 生成稳定的会话指纹
        let session_id_str = crate::modules::proxy::session_manager::SessionManager::extract_session_id(&request_for_body);
        ...
        if let Some(task_type) = background_task_type {
            ...
            for msg in request_with_mapped.messages.iter_mut() {
                if let crate::modules::proxy::mappers::claude::models::MessageContent::Array(blocks) = &mut msg.content {
                    blocks.retain(|b| !matches!(b, 
                        crate::modules::proxy::mappers::claude::models::ContentBlock::Thinking { .. } |
                        crate::modules::proxy::mappers::claude::models::ContentBlock::RedactedThinking { .. }
                    ));
                }
            }
        } else {
        ...
        if status.is_success() {
            ...
            if actual_stream {
                ...
                if client_wants_stream {
                    ...
                    use crate::modules::proxy::mappers::claude::collect_stream_to_json;
                ...
                let gemini_response: crate::modules::proxy::mappers::claude::models::GeminiResponse = match serde_json::from_value(raw.clone()) {
                    Ok(r) => r,
...
pub async fn handle_list_models(State(state): State<AppState>) -> impl IntoResponse {
    use crate::modules::proxy::common::model_mapping::get_all_dynamic_models;

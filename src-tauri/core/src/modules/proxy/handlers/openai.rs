...
use crate::modules::proxy::mappers::openai::{
    transform_openai_request, transform_openai_response, OpenAIRequest,
};
// use crate::modules::proxy::upstream::client::UpstreamClient; // 通过 state 获取
use crate::modules::proxy::server::AppState;

const MAX_RETRY_ATTEMPTS: usize = 3;
use crate::modules::proxy::session_manager::SessionManager;
...
pub async fn handle_chat_completions(
    ...
    if openai_req.messages.is_empty() {
        ...
        openai_req
            .messages
            .push(crate::modules::proxy::mappers::openai::OpenAIMessage {
                role: "user".to_string(),
                content: Some(crate::modules::proxy::mappers::openai::OpenAIContent::String(
                    " ".to_string(),
                )),
    ...
    for attempt in 0..max_attempts {
        // 2. 模型路由解析
        let mapped_model = crate::modules::proxy::common::model_mapping::resolve_model_route(
            &openai_req.model,
        ...
        let config = crate::modules::proxy::mappers::common_utils::resolve_request_config(
            &openai_req.model,
            &mapped_model,
            &tools_val,
        );
        ...
        if status.is_success() {
            // 5. 处理流式 vs 非流式
            if actual_stream {
                use crate::modules::proxy::mappers::openai::streaming::create_openai_sse_stream;
                use axum::body::Body;
                ...
                if client_wants_stream {
                    ...
                    use crate::modules::proxy::mappers::openai::collect_openai_stream_to_json;
        ...
        if status.is_success() {
            if list_response {
                ...
                let body = if is_codex_style {
                    use crate::modules::proxy::mappers::openai::streaming::create_codex_sse_stream;
                    let s =
                        create_codex_sse_stream(Box::pin(gemini_stream), openai_req.model.clone());
                    Body::from_stream(s)
                } else {
                    use crate::modules::proxy::mappers::openai::streaming::create_legacy_sse_stream;
                    let s =
            ...
            let choices = chat_resp.choices.iter().map(|c| {
                json!({
                    "text": match &c.message.content {
                        Some(crate::modules::proxy::mappers::openai::OpenAIContent::String(s)) => s.clone(),
                        _ => "".to_string()
                    },
                    "index": c.index,
                    "logprobs": null,

...
fn map_model_for_zai(original: &str, state: &crate::modules::proxy::ZaiConfig) -> String {
...
fn build_client(
    upstream_proxy: Option<crate::modules::proxy::config::UpstreamProxyConfig>,
    timeout_secs: u64,
...
pub async fn forward_anthropic_json(
    ...
    if !zai.enabled || zai.dispatch_mode == crate::modules::proxy::ZaiDispatchMode::Off {
